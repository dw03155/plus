var ItemSubtask = (function () {
    ...
    return {
        ...
        isSubtask: isSubtask,
        isSubTaskClickEvent: isSubTaskClickEvent,
    ...
    function isSubTaskClickEvent($eTarget) {
        var $subtaskArea = $eTarget.findUp(".js-subtask-area");
        if (!$subtaskArea || $subtaskArea.length === 0) return false;
        setTargetLayerAndOption();

        //Note. 엔터버튼은 작성권한과 무관!
        //Note. 기존 운영 기준 Status변경도 작성권한에 따라서 달라져서 우선 변경 처리!
        if (isEnterButtonAndAction($eTarget)) return true;

        if (!isAvailableWrite(getRegisterId($eTarget))) return true;

        if (isStatusAndAction($eTarget)) return true;
        if (isSubtaskButtonAndAction($eTarget)) return true;
        if (isDatePickerAndAction($eTarget)) return true;
        if (isPriorityAndAction($eTarget)) return true;
        if (isWorkerAndAction($eTarget)) return true;
        if (isSubtaskRemoveButtonAndAction($eTarget)) return true;
        if (isDetailSubtaskAndAction($eTarget)) return true;

        function setTargetLayerAndOption() {
            $popupBefore = $eTarget.findUp(".js-popup-before");
            isSubtaskEditLayer = $eTarget.findUp(".js-subtask-edit-area").length > 0;
            isViewPost = ($popupBefore.attr("data-code") === "VIEW");
            isEditPost = ($popupBefore.attr("data-code") === "EDIT");
            taskSrno = $eTarget.findUp(".js-subtask-li").attr("task-srno");
        }
    }

    function isSubtaskButtonAndAction($eTarget) {
        var $subtaskButton = $eTarget.findUp(".js-add-subtask-button");
        if (!$subtaskButton || $subtaskButton.length === 0) return false;

        if (LimitGuest.isLimitGuest("subtask")) return true;
    ...
    function isDetailSubtaskAndAction($eTarget) {
        var $subtaskLi = $eTarget.findUp(".js-subtask-li");
        var isEditArea = $eTarget.findUp(".js-subtask-edit-area").length > 0;
        if (!isViewPost || isEditArea || (!$subtaskLi || $subtaskLi.length === 0)) return false;
    ...
    function isStatusAndAction($eTarget) {
        ...
        var $statusLayer = $eTarget.findUp(".js-subtask-status-layer");
        if (!$statusLayer || $statusLayer.length === 0) return false;

        var $subtaskLi = $eTarget.findUp(".js-subtask-li");
        var $statusButton = $eTarget.findUp(".js-subtask-status-button");
        var $statusSettingButton = $eTarget.findUp(".js-status-setting-button");
    ...
    function isPriorityAndAction($eTarget) {
        var $subtaskLi = $eTarget.findUp(".js-subtask-li");
        var $priorityButton = $eTarget.findUp(".js-priority-button:visible");
        var $prioritySettingButton = $eTarget.findUp(".js-priority-setting-button");
        var $prioritySettingLayer = $subtaskLi.find(".js-priority-setting-layer");

        if ($priorityButton.length === 0 && $prioritySettingButton.length === 0) return false;
    ...
    function isWorkerAndAction($eTarget) {
        var $workerLayer = $eTarget.findUp(".js-subtask-worker-layer");
        if (!$workerLayer || $workerLayer.length === 0) return false;

        var isVisiblePopup = $("#workerPopup").is(":visible");
        var $subtaskLi = $eTarget.findUp(".js-subtask-li");
        var $workerButton = $eTarget.findUp(".js-worker-button");
        closeSubtaskPopup(isVisiblePopup ? "ALL" : "WORKER");
        if ($workerButton.length > 0) {
            !isVisiblePopup && $eTarget.findUp(".js-subtask-li").addClass("active");
    ...
    function isDatePickerAndAction($eTarget) {
        var $dateLayer = $eTarget.findUp(".js-subtask-date-layer");
        if (!$dateLayer || $dateLayer.length === 0) return false;

        closeSubtaskPopup("ALL");
        var $pickerObj = $eTarget.findUp(".js-pickr-layer");
        $pickerObj.findUp(".js-subtask-li").addClass("active");
        addDatePickerOnSubtask($pickerObj);
        return true;
    }

    function isSubtaskRemoveButtonAndAction($eTarget) {

        var $removeButton = $eTarget.findUp(".js-subtask-remove");
        if (!$removeButton || $removeButton.length === 0) return false;
        ...
        function subtask() {
            var $subtaskUl = $eTarget.findUp(".js-subtask-ul");
            $removeButton.findUp(".js-subtask-li").remove();
            getSubtaskCount($subtaskUl);
        }
    }

    function isEnterButtonAndAction($eTarget) {
        var $enterButton = $eTarget.findUp(".js-subtask-enter-button");
        if (!$enterButton || $enterButton.length === 0) return false;
        var $editLayer = $eTarget.findUp(".js-subtask-edit-layer");
        var $subtaskLi = $enterButton.findUp(".js-subtask-li");
        ...
        addAndDrawSubtask(addSubtask, subtaskData, $eTarget);
    ...
    function addDatePickerOnSubtask($inputTarget) {
        var $subtaskLi = $inputTarget.findUp(".js-subtask-li");
        var $dateLayer = $inputTarget.findUp(".js-subtask-date-layer");
        var $pickrLayer;
        var selectDate;
        var isSame = false;
        ...
        function updateCallback() {
            ...
            function setDate() {
                var $dateLayer = $pickrLayer.findUp(".js-subtask-date-layer");
    ...
    function makeSubtaskRecord($postPopup, isEdit) {
        ...
        var isViewPost = PostCondition.getPopupMode($postPopup) === "VIEW";
    ...
    function getSubtaskJson($subtask) {
        var isEditArea = $subtask.findUp(".js-subtask-edit-area").length > 0;
        ...
        var $worker = $subtask.find(".js-subtask-worker-layer");
    ...
    function addSortableAction($subtaskUl) {
        $.each($subtaskUl, function (i, $ul) {
            ...
            isSortable && $($ul).sortable({
                ...
                stop: function (e, ui) {
                    var $item = $(ui.item);
                    var $popupBefore = $item.findUp(".js-popup-before");
    ...
    function drawPriorityLayerAndPosition($button, $layer) {
        ...
        var $postPopup = $button.findUp("#postPopup");
        var isPostPopup = $postPopup.length > 0;
        var $popupBefore = $button.parents(".js-popup-before");
        var $subtaskOptionLayer = $button.findUp(".js-subtask-layer");
        ...
        $button.findUp(".js-subtask-li").addClass("active");
        return {
    ...
    function isSubtaskInputFocus(e) {
        ...
        var $subtaskInput = $eTarget.findUp(".js-subtask-input");
