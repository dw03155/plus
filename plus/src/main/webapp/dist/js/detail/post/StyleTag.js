var StyleTag = (function () {
    ...
    function keyUpCheck(e) {
        if (KeyCheck.isKey(e, "BACK") || KeyCheck.isKey(e, "DELETE")) closeStyleTagPopup();
        controlStyleTagPop(e);
    }
    ...
    function controlStyleTagPop(e) {
        ...
        if (!selection || selection.rangeCount !== 1 || !isAvailable(selection)) return closeStyleTagPopup();
        if (selection.anchorNode.nodeType !== Node.TEXT_NODE) return closeStyleTagPopup();
    ...
    function controlSelection(e) {
        ...
        Often.clog("controlSelection", selection);
    ...
    function openStyleTagPopup(range) {
        ...
        $styleTagItem.css({top: position.y + "px", left: position.x + "px"});
        findCurrentTags();
