...
<body>
	...
	<div class="main-container">
			...
			<div id="detailTop" class="project-detail title-1">
				<div class="project-detail-header">
					<div class="project-color-area">
						<i id="projectColor" class="project-color color-code-${prjInfo.prjColor}"></i>
					</div>
							...
							<div class="project-option-area">
								...
								<div id="detailSettingLayer" class="project-setup-wrap" style="display: none">
									...
									<ul id="detailSettingGroup" class="setup-group">
										<li id="detailSettingColorBtn"><a href="#"> <i class="icon-set-color"></i>색상 설정
											</a></li>
										<li id="detailSettingLabelBtn"><a href="#"> <i class="icon-set-label"></i>프로젝트
												폴더 설정
											</a></li>
										<li id="detailSettingPushAlarmBtn"><a href="#"> <i class="icon-set-alarm"></i>알림
												설정
											</a></li>
							...
							<ul class="project-status-group">
								<c:if test="${prjInfo.prjKnd == 'N'}">
									<li id="lockIcon" class="d-none"><i
											class="sprite-detail icon-locked js-icon-locked"><span class="blind">일반
												프로젝트</span></i>
								...
								<c:if test="${prjInfo.prjKnd == 'C'}">
									<li id="companyIcon" class="d-none"><i
											class="sprite-detail icon-company js-icon-company"><span class="blind">회사
												프로젝트</span></i>
										<div class="tooltip-square">
		...
		<div id="mainContent" class="main-content scroll-mask" scroll-direction="0">
			<div id="detailLayer" class="main-sub-header project-detail-wrap d-none" style="display: block">
				...
				<!-- 프로젝트 홈탭 -->
				<div class="project-detail-inner layer-scroll type2" style="display: block">
							...
							<div class="project-detail-content">
								...
								<div>
									<div class="detail-section reports-section">
										...
										<!-- 원형차트 -->
										<div id="doughnutChartLayer" class="js-task-report-layer d-none"
											style="display: block">
											<div class="taks-report">
												...
												<ul id="taskReportLayer" class="donut-chart-list">
													<li>
														<c:forEach var="tskPrgsCnt" items="${tskPrgsCnts}">
															...
															<span class="task-chart-info ${tskChartClass}"> <i
																...
																<span class="chart-info-percent">
																	<fmt:formatNumber type="number"
																		value="${tskPrgsCnt.tskCnt / tskAllCnt * 100}" />%</span>
															</span>
														</c:forEach>
													</li>
												</ul>
								...
								<div id="createPostArea" class="work-design-wrapper">
									<ul id="createPostUl" class="work-design-group">
										...
										<li class="post-filter" data-post-code="2"><i class="icons-todo"></i><span>할
												일</span></li>
									</ul>
								...
								<ul id="pinPostUl" class="pin-list fixed-list">
									<c:forEach var="pincette" items="${pincettes}">
										<li class="js-pin-item"><a href="#">
												...
												<c:if test="${pincette.notiKnd=='subtask'}">
													<c:set var="notiKndIcon" value="icons-task" />
												...
												class="feedback"피드백 -->
													...
													<c:if test="${pincette.notiKnd=='schedule'}">
														<div class="date-time">
															<em class="date">
																<fmt:parseDate value="${pincette.addList}"
																	pattern="YY/MM/dd" var="addDate" />
																<fmt:formatDate value="${addDate}" pattern="MM/dd" />
															</em> <span>
																<fmt:parseDate pattern="YY/MM/dd"
																	value="${pincette.addList}" var="addTime" />
																<fmt:formatDate value="${addTime}" pattern="HH:mm" />
															</span>
														</div>
													</c:if>
													<!-- 할일일 때 -->
													<c:if test="${pincette.notiKnd=='todo'}">
														<span
															class="js-task-state js-todo-state state request">${pincette.addList}%</span>
													</c:if>
												</div>
											</a></li>
									</c:forEach>
								...
								<div class="feed-content">
									<ul id="detailUl" class="post-group list" style="display: block">
										...
										<c:forEach var="nwList" items="${nwLists}">
											<li id="myPcontent" class="js-popup-before detail-item back-area">
																...
																<div class="fixed-value">
																	...
																	<c:if test="${nwList.notiKnd=='schedule'}">
																		<div class="js-schedule-state date-time d-none"
																			style="display: block">
																			<em class="date">
																				<fmt:parseDate value="${nwList.addList}"
																					pattern="YY/MM/dd" var="addDate" />
																				<fmt:formatDate value="${addDate}"
																					pattern="MM/dd" /></em> <span>
																				<fmt:parseDate pattern="YY/MM/dd"
																					value="${nwList.addList}"
																					var="addTime" />
																				<fmt:formatDate value="${addTime}"
																					pattern="HH:mm" /></span>
																		</div>
																	</c:if>
																	<!-- 할일일 때 -->
													...
													<li class="js-participant-item">
														<div class="post-author">
															...
															<dl class="post-author-info">
																<dt>
																	<strong
																		class="js-participant-name author ellipsis">${pm.name}</strong>
													...
													<li class="js-participant-item">
														<div class="post-author">
															...
															<dl class="post-author-info">
																<dt>
																	<strong
																		class="js-participant-name author ellipsis">${user.name}</strong>
													...
													<li class="js-participant-item">
														<div class="post-author">
															...
															<dl class="post-author-info">
																<dt>
																	<strong
																		class="js-participant-name author ellipsis">${guest.name}</strong>
																	<em class="position ellipsis">${guest.wkpo}</em>
																</dt>
																<dd>
																	<strong class="company">${guest.coUrl}</strong>
																	<span class="team">${guest.dept}</span>
																</dd>
															</dl>
														</div>
														<button type="button"
				...
				<div class="js-post-search-result all-search-section d-none me-post-wrap" style="display: none">
						...
						<div class="all-search-content">
							<div id="postSearchArea" class="search-result-group">
								<div class="search-title-area">
									<span class="search-result-title">전체</span> <span id="allPostsSearchCount"
										class="js-search-post-count search-result-count"
										style="display: inline-block">${fn:length(nwLists)}</span>
								</div>
								<ul id="allPostsSearchUl" class="js-search-post-ul all-seach-list-type-1 scroll-mask">
									<!-- 반복 시작 -->
									<c:forEach var="nwList" items="${nwLists}">
					...
					<div class="flow-content scroll-mask">
						<div class="flow-content-1">
							<input id="projectTitleInput" type="text" placeholder="제목을 입력하세요." maxlength="50"
								autocomplete="off" data-empty-msg="제목을 입력하세요." data-over-msg="제목은 50자 이하로 입력하세요."
								data-required-yn="Y">
						</div>
						...
						<div class="flow-content-3">
							옵션
							<button class="js-service-helper js-mouseover" service-code="OPTION">
								<i class="icons-help"></i>
							</button>
						</div>
						<a href="#"> </a>
						<div class="open-yn check-setting flow-content-4" style="display: none">
			...
			<div class="flow-project-make-2">
				<div id="inviteMainLayer" class="detail-popup-type-1" style="display: none">
					...
					<div class="detail-popup-header-1">
						<span id="inviteTitle" class="invite-title ellipsis"><i
								class="project-color color-code-9"></i></span> <a class="closeInviteLayerBtn"
							href="#"><em></em></a>
					</div>
					<ul id="inviteUl">
				...
				<div id="teamInviteLayer" class="name-type-seach-popup-type-1" style="display: none">
					<div class="name-type-seach-popup-header-type-1">
						<a href="#"><em class="returnMainBtn"></em></a> <span id="teamInviteHeader">회사 직원 초대</span>
						<button class="btn-close closeInviteLayerBtn">
							<i class="icons-close-1"></i>
						</button>
					</div>
					<div class="all-setup-type-2">
						<i class="icons-search"></i> <input type="text" id="teamInviteSearch"
							class="coperate-input-type-1" placeholder="이름 소속 연락처 내선 검색" autocomplete="off">
					</div>
					...
					<div id="teamInviteArea" class="group-tree-wrap-1 coperate-section-position-fix-1 scroll-mask">
						<ul id="inviteOrgChart" class="d-none">
						</ul>
						<ul id="inviteMemberList" class="participants-list invite-member-list">
						</ul>
					</div>
					<div id="inviteEmplArea" class="sub-drag-section-2 invite-empl-area coperate-section-position-fix-1"
						style="display: none;">
						<div class="line-fixed-section-1"></div>
						<div class="sub-drag-header-type-2">
							<span id="emplList-dvsnName"></span> <span id="resultSearch"
								class="empl-search-text d-none">검색 결과</span> <a id="emplAreaCloseBtn" href="#"
								class="group-close-type-1"></a>
						</div>
	...
	<div id="tempEmailItem" style="display: none">
		<div class="input-email-type-wrap-1 emailItem">
			<input type="text" class="input-email-type-1 emailItemInput" placeholder="example@flow.team"
				...
				data-over-msg="이메일은 50자이내로 작성해주세요!" data-un-valid-msg="올바른 이메일을 작성해주세요!"> <a class="deleteEmail"
				href="#"></a>
		</div>
	</div>
	<div id="selectMemberItem" style="display: none">
		<li rcvr_cd="{rcvr-cd}" rcvr_gb="{rcvr-gb}" id="{id}"><i {profile}=""></i> <a href="#"> <span
		...
		<div class="flow-project-make-1">
			<div class="flow-project-make-2">
				<div id="allSendienceLayer" class="project-invite-popup-1">
					...
					<div class="all-setup-type-2">
						<i class="icons-search"></i> <input type="text" id="allSendienceSearch"
							class="coperate-input-type-1" placeholder="참여자명으로 검색">
					</div>
