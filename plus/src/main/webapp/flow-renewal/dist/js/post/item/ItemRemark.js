var ItemRemark = (function () {
    ...
    return {
        //event
        clickRemarkComponent: clickRemarkComponent,
        ...
        moveToRemarkBottom: moveToRemarkBottom,
    }

    function clickRemarkComponent(e) {
        var $eTarget = $(e.target);
        var $remarkLayer = $eTarget.closest(".js-remark-layer");
        if ($.isTarget(e, ".js-remark-upload-button")) return clickFileUpload($remarkLayer);
        if (isCommentWriterButtonAndAction($eTarget)) return;
        if (isRemarkDeleteFileButtonAndAction($eTarget)) return;
        if (isShowPrevRemarkButtonAndAction($eTarget)) return;
        if (isPostCommentGroup($eTarget)) return;
        if (isRemarkLikeButtonAndAction($eTarget)) return;
        if (isRemarkImgAndAction($eTarget)) return;
        if (isRemarkFileAndAction($eTarget)) return;
        if (isRemarkLikeCountButtonAndAction($eTarget)) return;
        if (isShowMoreRemarkButtonAndAction($eTarget)) return;
    }
    ...
    function moveToRemarkBottom(e) {
        var $eTarget = $(e.target);
        var $postPopup = $eTarget.findUp("#postPopup");
        var $remarkArea = $eTarget.findUp(".js-remark-layer");
        if (!$.isTarget(e, "#postPopup")) return;
        if (!$remarkArea.hasClass("sticky")) return;
        var $postScroll = $postPopup.find(".post-card-scroll");
        $postScroll.moveToBottom(true);
    }
