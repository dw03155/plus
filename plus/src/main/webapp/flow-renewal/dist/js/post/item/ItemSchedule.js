var ItemSchedule = (function () {
    ...
    return {
        ...
        clickScheduleComponent: clickScheduleComponent,
    ...
    function clickScheduleComponent(e) {
        var $eTarget = $(e.target);
        var $popupBefore = $eTarget.findUp(".js-popup-before");
        var isSchedulePost = $popupBefore.find(".js-post-nav").hasClass("schedule");
        if (!isSchedulePost) return;

        isViewPost = ($popupBefore.attr("data-code") === "VIEW");
        if (isVideoButtonAndAction($eTarget)) return
        if (isCancelVideoButtonAndAction($eTarget)) return;
        if (isOneDayButtonAndAction($eTarget)) return;
        if (isAttendanceAndAction($eTarget)) return;
        if (isAttendanceSelectAndAction($eTarget)) return;
        if (isPlaceSpanAndAction($eTarget)) return;
        if ($.isTarget(e, ["#zoomUrlCopy", "#zoomButton"])) return VideoConference.clickZoomComponent(e);
    }
