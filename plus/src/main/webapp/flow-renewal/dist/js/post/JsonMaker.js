var JsonMaker = (function () {
    ...
    function getOriginalPostContentsJson() {
        ...
        function getOriginalTextJson() {
            ...
            $.each($($uploadArea[0].childNodes), function (index, node) {
                if (Often.isBrowser("ie") && node.nodeName === "P") {
                    node = changeDivNode(node, index);
                }
    ...
    function changeDivNode(node, index) {
        var div = document.createElement("div");
        div.innerHTML = node.innerHTML;
        node.parentNode.replaceChild(div, node.parentNode.childNodes[index]);
        return div;
    }
