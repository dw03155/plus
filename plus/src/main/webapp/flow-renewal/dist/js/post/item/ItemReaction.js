var ItemReaction = (function () {
    ...
    return {
        clickReactionArea: clickReactionArea,
        closeAllReactionPopup: closeAllReactionPopup,
        updateReactionElements: updateReactionElements,
    }

    function clickReactionArea(e) {
        var $eTarget = $(e.target);
        if (Often.isItem($eTarget, ".js-post-reaction")) return clickReactionButton($eTarget);
        if (Often.isItem($eTarget, ".js-emoji-select-layer")) return isEmojiIconAndAction($eTarget);
        if (Often.isItem($eTarget, ".js-emoji-group-layer")) return isEmojiGroupAreaAndAction($eTarget);
    }

    function clickReactionButton($eTarget) {
